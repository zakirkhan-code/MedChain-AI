...
exports.submitApplication = async (req, res) => {
  try {
    const { specialization, licenseNumber, credentials, documentURI } =
      req.body;
    ...
    if (!specialization || !licenseNumber) {
      return res
        ...
        .json({
          success: false,
          message: "Specialization and license number required",
        });
    }
    ...
    if (user.role !== "doctor")
      return res
        ...
        .json({
          success: false,
          message: "Only doctors can submit application",
        });
    ...
    if (user.walletAddress) {
      blockchainData = blockchain.prepareDoctorApplication({
        licenseNumber,
        credentials,
        specialization,
      });
      ...
      blockchainData.documentURI =
        documentURI || `ipfs://medchain/doctor-docs/${Date.now()}`;
      console.log(
        `📋 Doctor ${user.name} application saved in DB. On-chain submission pending from app.`,
      );
    }
    ...
    res.json({
      ...
      message:
        "Application submitted in DB. Complete on-chain registration from app using blockchainData.",
    });
  } catch (err) {
...
exports.confirmOnChain = async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: { user },
      message:
        "On-chain registration confirmed. Waiting for admin verification.",
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
exports.listDoctors = async (req, res) => {
  try {
    ...
    const doctors = await User.find(query)
      .select(
        "name email phone specialization licenseNumber rating ratingCount totalPatients walletAddress isVerified onChainStatus",
      )
...
exports.getDoctorById = async (req, res) => {
  try {
    const doctor = await User.findOne({
      ...
      "name email phone specialization licenseNumber credentials rating ratingCount totalPatients walletAddress isVerified onChainStatus txHashes",
    );
    if (!doctor)
      return res
        .status(404)
...
exports.rateDoctor = async (req, res) => {
  try {
    ...
    if (!doctor.isVerified)
      return res
        .status(400)
        .json({ success: false, message: "Can only rate verified doctors" });

    // Prevent self-rating
    if (doctor._id.toString() === req.user._id.toString()) {
      return res
        .status(400)
        .json({ success: false, message: "Cannot rate yourself" });
    }

    const newCount = (doctor.ratingCount || 0) + 1;
    const newRating =
      ((doctor.rating || 0) * (doctor.ratingCount || 0) + Number(rating)) /
      newCount;
...
exports.getDoctorStats = async (req, res) => {
  try {
    ...
    const activeAccess = await AccessLog.countDocuments({
      provider: user._id,
      action: "granted",
    });
