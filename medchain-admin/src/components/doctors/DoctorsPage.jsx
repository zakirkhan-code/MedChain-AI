...
import { Users, Search, CheckCircle, XCircle, Ban, Eye, Loader2, ExternalLink, RefreshCw } from "lucide-react";
import { adminAPI } from "../../services/api";
import { verifyDoctorOnChain, rejectDoctorOnChain, suspendDoctorOnChain, grantDoctorRole, getDoctorStatus, DOCTOR_STATUS } from "../../services/blockchain";
...
export default function DoctorsPage() {
  ...
  const handleVerify = async (id) => {
    ...
    try {
      ...
      // Try on-chain first (MetaMask)
      if (window.ethereum && selected?.walletAddress) {
        try {
          const chainStatus = await getDoctorStatus(selected.walletAddress);
          if (chainStatus === 1) {
            const result = await verifyDoctorOnChain(selected.walletAddress);
            txHash = result.txHash;
            onChainSuccess = true;

            // Also grant DOCTOR_ROLE
            try {
              await grantDoctorRole(selected.walletAddress);
            } catch (roleErr) {
              console.warn("Grant role failed:", roleErr.message);
            }
          } else {
            toast.error(`Doctor on-chain status is "${DOCTOR_STATUS[chainStatus] || 'Unknown'}" — must be "Pending" for on-chain verify`);
          }
        } catch (chainErr) {
          console.warn("On-chain verify failed:", chainErr.reason || chainErr.message);
          toast.error("On-chain failed: " + (chainErr.reason || chainErr.message));
        }
      }

      // Backend verify (always)
      await adminAPI.verifyDoctor(id);

      if (onChainSuccess) {
        toast.success(`Doctor verified (On-chain ✓)\nTx: ${txHash?.slice(0, 16)}...`);
      } else {
        toast.success("Doctor verified (Off-chain only)");
      }
    ...
    try {
      ...
      if (window.ethereum && selected?.walletAddress) {
        try {
          const chainStatus = await getDoctorStatus(selected.walletAddress);
          if (chainStatus === 1) {
            await rejectDoctorOnChain(selected.walletAddress, reason);
            onChainSuccess = true;
          }
        } catch (chainErr) {
      ...
      toast.success(onChainSuccess ? "Doctor rejected (On-chain ✓)" : "Doctor rejected (Off-chain)");
      fetchDoctors(); setModal(null); setReason(""); setSelected(null);
    } catch (err) {
  ...
  const handleSuspend = async (id) => {
    ...
    try {
      ...
      if (window.ethereum && selected?.walletAddress) {
        try {
          await suspendDoctorOnChain(selected.walletAddress, reason);
          onChainSuccess = true;
        } catch (chainErr) {
          console.warn("On-chain suspend failed:", chainErr.reason || chainErr.message);
        }
      }

      await adminAPI.suspendDoctor(id, { reason });
      toast.success(onChainSuccess ? "Doctor suspended (On-chain ✓)" : "Doctor suspended (Off-chain)");
      fetchDoctors(); setModal(null); setReason(""); setSelected(null);
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed");
    }
  ...
  const statusBadge = (d) => {
    if (d.isVerified) return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-emerald-100 text-emerald-700">Verified</span>;
    if (d.specialization) return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-amber-100 text-amber-700">Pending</span>;
    return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-slate-100 text-slate-600">Registered</span>;
  };

  const chainBadge = (walletAddress) => {
    const s = chainStatuses[walletAddress];
    if (s === undefined) return null;
    const colors = { 0: "bg-slate-100 text-slate-600", 1: "bg-blue-100 text-blue-700", 2: "bg-emerald-100 text-emerald-700", 3: "bg-red-100 text-red-700", 4: "bg-orange-100 text-orange-700" };
    return <span className={`px-2 py-1 text-xs font-semibold rounded-full ${colors[s] || colors[0]}`}>Chain: {DOCTOR_STATUS[s]}</span>;
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Doctors Management</h2>
          <p className="text-sm text-slate-500 mt-1">{filtered.length} doctors found</p>
        </div>
        <button onClick={fetchDoctors} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">Doctor</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">Specialization</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">License</th>
              ...
              {filtered.map(d => (
                <tr key={d._id} className="hover:bg-slate-50">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-slate-400">{d.email || d.cnic}</p>
                    {d.walletAddress && <p className="text-xs text-blue-500 font-mono mt-0.5">{d.walletAddress.slice(0, 10)}...</p>}
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-600">{d.specialization || "—"}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{d.licenseNumber || "—"}</td>
                  <td className="px-4 py-3">{statusBadge(d)}</td>
                  <td className="px-4 py-3">
                    {chainBadge(d.walletAddress)}
                    {d.walletAddress && !chainStatuses[d.walletAddress] && chainStatuses[d.walletAddress] === undefined && (
                      <button onClick={() => fetchChainStatus(d.walletAddress)} className="text-xs text-blue-500 hover:underline">Check</button>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => { setSelected(d); setModal("view"); if(d.walletAddress) fetchChainStatus(d.walletAddress); }}
                      ...
                      {!d.isVerified && d.specialization && (
                        <>
                          <button onClick={() => { setSelected(d); setModal("verify"); }} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Verify">
                            <CheckCircle className="w-4 h-4" />
                          </button>
                          <button onClick={() => { setSelected(d); setModal("reject"); }} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg" title="Reject">
                            <XCircle className="w-4 h-4" />
                          </button>
                        </>
                      )}
                      {d.isVerified && (
                        <button onClick={() => { setSelected(d); setModal("suspend"); }} className="p-1.5 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg" title="Suspend">
      ...
      {modal && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => { setModal(null); setReason(""); }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            {modal === "view" && (
              <>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div><span className="text-slate-400">Specialization:</span> {selected.specialization || "—"}</div>
                  <div><span className="text-slate-400">License:</span> {selected.licenseNumber || "—"}</div>
                  <div><span className="text-slate-400">Credentials:</span> {selected.credentials || "—"}</div>
                  <div><span className="text-slate-400">Wallet:</span> <span className="font-mono text-xs">{selected.walletAddress || "—"}</span></div>
                  <div><span className="text-slate-400">DB Status:</span> {statusBadge(selected)}</div>
            ...
            {modal === "verify" && (
              <>
                <h3 className="text-lg font-bold text-slate-900 mb-2">Verify Doctor</h3>
                <p className="text-sm text-slate-500 mb-4">Verify <strong>{selected.name}</strong> ({selected.specialization})?</p>

                {!window.ethereum && (
                  <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded-lg mb-3">⚠️ MetaMask not found. Verification will be off-chain only.</p>
                )}
                {window.ethereum && !selected.walletAddress && (
                  <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded-lg mb-3">⚠️ Doctor has no wallet. Verification will be off-chain only.</p>
                )}
                {window.ethereum && selected.walletAddress && (
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleVerify(selected._id)} disabled={actionLoading}
                    ...
                    {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />} Verify
                  </button>
                </div>
              </>
            )}
            ...
            {modal === "reject" && (
              <>
                <h3 className="text-lg font-bold text-slate-900 mb-2">Reject Doctor</h3>
            ...
            {modal === "suspend" && (
              <>
                <h3 className="text-lg font-bold text-slate-900 mb-2">Suspend Doctor</h3>
                <p className="text-sm text-slate-500 mb-3">Suspend <strong>{selected.name}</strong>?</p>
                <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Reason for suspension..."
                  className="w-full p-3 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 mb-3" rows={3} />
                <div className="flex gap-2">
                  <button onClick={() => { setModal(null); setReason(""); }} className="flex-1 py-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 text-sm font-medium">Cancel</button>
                  <button onClick={() => handleSuspend(selected._id)} disabled={actionLoading}
                    ...
                    {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Ban className="w-4 h-4" />} Suspend
                  </button>
                </div>
              </>
            )}
