...
export default function PatientsPage() {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const res = await adminAPI.approvePatient(id);
      toast.success(res.data.message || "Patient approved!");
      fetchPatients();
      setModal(null);
      setSelected(null);
    } catch (err) {
  ...
  const handleReject = async (id) => {
    ...
    setActionLoading(true);
    try {
      const res = await adminAPI.rejectPatient(id, { reason });
      toast.success(res.data.message || "Patient rejected");
      fetchPatients();
      setModal(null);
      setReason("");
      setSelected(null);
  ...
  const statusBadge = (status) => {
    ...
    return (
      <span
        className={`px-2 py-1 text-xs font-semibold rounded-full ${map[status] || map[0]}`}
      >
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((p) => (
                <tr key={p._id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    {statusBadge(p.onChainStatus || 0)}
                  </td>
                  <td className="px-4 py-3">
                    {chainBadge(p.walletAddress)}
                    {p.walletAddress &&
                      chainStatuses[p.walletAddress] === undefined && (
                        <button
                          onClick={() => fetchChainStatus(p.walletAddress)}
                    ...
                    <div className="flex items-center justify-end gap-1">
                      <button
                        onClick={() => {
                          ...
                          setModal("view");
                          if (p.walletAddress)
                            fetchChainStatus(p.walletAddress);
                        }}
              ...
              <>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div>
                    ...
                    {statusBadge(selected.onChainStatus || 0)}
                  </div>
                  <div>
            ...
            {modal === "approve" && (
              <>
                ...
                <p className="text-sm text-slate-500 mb-4">
                  Approve <strong>{selected.name}</strong> registration?
                </p>

                {!window.ethereum && (
                  <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded-lg mb-3">
                    ⚠️ MetaMask not found. Approval will be off-chain only.
                  </p>
                )}
                {window.ethereum && !selected.walletAddress && (
                  <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded-lg mb-3">
                    ⚠️ Patient has no wallet. Approval will be off-chain only.
                  </p>
                )}
                {window.ethereum && selected.walletAddress && (
